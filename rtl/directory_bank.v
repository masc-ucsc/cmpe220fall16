...
typedef struct packed {
  logic [`OUTSTANDING_REQUEST_BITS-1:0] req_pos; //the outstanding request position
...
typedef struct packed {
  ...
  SC_cmd_type       cmd;
  SC_paddr_type     paddr;
  logic [`OUTSTANDING_REQUEST_BITS-1:0] next;
} I_dr_req_queue_type;
// 1}}}
...
`ifndef DR_PASSTHROUGH
  ...
  I_dr_req_queue_type req_queue [0:MAX_OUTSTANDING_REQUESTS-1];
  I_dr_req_queue_type req_queue_next [0:MAX_OUTSTANDING_REQUESTS-1];
  
  genvar k;  
  generate  
    for (k=0; k < MAX_OUTSTANDING_REQUESTS; k=k+1) begin: req_queue_gen 
      flop_r #(.Size($bits(I_dr_req_queue_type)), .Reset_Value('b0))
        flop_queue_buf_inst 
        (
           .clk(clk)
          ,.reset(reset)
          ,.din(req_queue_next[k])
          ,.q(req_queue[k])
        ); 
        
        flop_r #(.Size(DR_ENTRY_WIDTH + 2), .Reset_Value('b0))
        flop_entry_queue_inst 
        (
           .clk(clk)
          ,.reset(reset)
          ,.din(entry_buf_next[k])
          ,.q(entry_buf[k])
        ); 
    end  
  endgenerate 
  ...
  logic we_queue;
  logic [`OUTSTANDING_REQUEST_BITS-1:0] pos_queue;
  
  always_comb begin
    we_queue = 'b0;
    pos_queue = 'b0;
    if(l2todr_req_valid && req_queue_valid && (!req_qstage_ff_next_retry)) begin
      we_queue = 1'b1;
      pos_queue = req_queue_valid_encoder;
    end else if(l2todr_snoop_ack_valid && !l2todr_snoop_ack_retry) begin
      pos_queue = l2todr_snoop_ack.drid[`OUTSTANDING_REQUEST_BITS-1:0];
    end
  end
  ...
  always_comb begin
    req_queue_next = req_queue;
    
    if(we_queue) begin
    //acquire space in buffer
      req_queue_next[pos_queue].valid = 1'b1;
      req_queue_next[pos_queue].paddr = l2todr_req.paddr;
      req_queue_next[pos_queue].l2id = l2todr_req.l2id;
      req_queue_next[pos_queue].nid = l2todr_req.nid;
      req_queue_next[pos_queue].cmd = l2todr_req.cmd;
      req_queue_next[pos_queue].next = 'b0;
    end
    ...
    //not affect behaviour.
      req_queue_next[req_queue_release_pos] = 'b0;
    end
  end
  
  I_dr_pipe_stage_type            req_qstage_ff_next;
  logic                           req_qstage_ff_next_valid;
  logic                           req_qstage_ff_next_retry;
  
  
  I_dr_pipe_stage_type            req_qstage_ff;
  logic                           req_qstage_ff_valid;
  logic                           req_qstage_ff_retry;
  
  always_comb begin
    req_qstage_ff_next.paddr = l2todr_req.paddr;
    req_qstage_ff_next.cmd   = l2todr_req.cmd;
    req_qstage_ff_next.drid  = {'b0,req_queue_valid_encoder};  
    req_qstage_ff_next.req_pos = 'b0; //no longer applicable
    req_qstage_ff_next.nid = l2todr_req.nid;
    req_qstage_ff_next.dr_cmd = 1'b0;
  end
  
  fflop #(.Size($bits(I_dr_pipe_stage_type))) req_qstage_fflop (
    .clk      (clk),
    .reset    (reset),

    .din      (req_qstage_ff_next),
    .dinValid (req_qstage_ff_next_valid),
    .dinRetry (req_qstage_ff_next_retry),

    .q        (req_qstage_ff),
    .qValid   (req_qstage_ff_valid),
    .qRetry   (req_qstage_ff_retry)
  );
  
  logic [`OUTSTANDING_REQUEST_BITS-1:0] req_queue_valid_encoder;
  logic req_queue_valid;
  I_dr_req_queue_type req_queue_value_temp;
  always_comb begin  
    ...
    req_queue_valid_encoder = 'b0;
    req_queue_valid = !req_queue[0].valid;
    while ((!req_queue_valid) && (req_queue_valid_encoder != MAX_REQ_BUF_VALUE)) begin
      req_queue_valid_encoder = req_queue_valid_encoder + 1;
      req_queue_valid = !req_queue[req_queue_valid_encoder].valid;
    end
  end
  ...
  //ALTER THIS
  //This needs to change in several ways. First, we are now saving l2id, nid in the outstanding request.
  ...
  always_comb begin
    ...
    //to set just that bit and this is simpler. Will come back to fix this later, but will
    //not affect behaviour.
      req_buf_next[req_buf_release_pos] = 'b0;
    end
  end
 ...
 //ALTER THIS
 //not too much, see above, make this change first and see if compiles
  ...
  assign l2todr_req_retry = !drid_valid || !req_buf_valid || l2todr_req_gen_retry || id_ram_write_next_retry;
  assign l2todr_req_gen_valid = l2todr_req_valid && drid_valid && req_buf_valid && !id_ram_write_next_retry;
  assign id_ram_write_next_valid = l2todr_req_valid && drid_valid && req_buf_valid && !l2todr_req_gen_retry;
  
  always_comb begin
    l2todr_req_gen.paddr = l2todr_req.paddr;
    l2todr_req_gen.cmd   = l2todr_req.cmd;
    l2todr_req_gen.drid  = drid_valid_encoder;  
    l2todr_req_gen.req_pos = req_buf_valid_encoder;
  ...
  always_comb begin
    ...
    end else if(l2todr_req_gen_valid) begin
...
//MEMTODR REQ ARBITER START
  ...
  always_comb begin  
    ...
    if(tag_gen_req_next_valid) begin
      ...
      req_buf_release_pos = req_gen_buf_release_pos;
      ...
      req_buf_release_pos = sa_req_buf_ack_pos;
    end
  end
...
//SNOOP ACK START
  ...
  //ALTER THIS
  //see notes, change the position based on drid
  always_comb begin
...
//L2TODR DISP START
  ...
  assign {disp_buf_valid, disp_buf} = req_buf[disp_req_buf_pos];
...
//SNACK ARBITER START
  ...
  always_comb begin
    ...
    if(req_gen_snack_valid) begin
      ...
      drtol2_snack_next_valid = 1'b1;
      mem_ack_gen_snack_retry = drtol2_snack_next_retry;
...
//MEMTODR ACK START
  ...
  always_comb begin
    if(memtodr_ack_ff.drid == {`DR_REQIDBITS{1'b0}}) begin
      ...
      //prefetch. 
      mem_ack_gen_snack.nid = memtodr_ack_ff.nid; 
      mem_ack_gen_snack.l2id = {`L2_REQIDBITS{1'b0}};
	    mem_ack_gen_snack.hpaddr_base = 'b0;
	    mem_ack_gen_snack.hpaddr_hash = 'b0;
      mem_ack_gen_snack.paddr = memtodr_ack_ff.paddr;
      
      memtodr_ack_ff_retry = mem_ack_gen_snack_retry;
      id_ram_retry = 1'b1;
    end else begin
      //If the DRID is valid then ignore the prefetch terms and nid, l2id are set by the RAM
      mem_ack_gen_snack.nid = id_ram_data[10:6]; //These needs to be changed to match the request nid and l2id.
      mem_ack_gen_snack.l2id = id_ram_data[5:0];
	    mem_ack_gen_snack.hpaddr_base = compute_dr_hpaddr_base(memtodr_ack_ff.paddr);
	    mem_ack_gen_snack.hpaddr_hash = compute_dr_hpaddr_hash(memtodr_ack_ff.paddr);
      //mem_ack_gen_snack.paddr = 'b0;
      //Paddr should be set to 0 but not doing this to allow testbench to pass for now...
      mem_ack_gen_snack.paddr = memtodr_ack_ff.paddr;
      
      memtodr_ack_ff_retry = mem_ack_gen_snack_retry || (!mem_ack_gen_snack_valid && memtodr_ack_ff_valid);
      id_ram_retry = mem_ack_gen_snack_retry || (!mem_ack_gen_snack_valid && id_ram_valid);
    end
  end
  ...
  always_comb begin
    if(memtodr_ack_ff.drid == {`DR_REQIDBITS{1'b0}}) begin
      //if drid is invalid then this is an ack for a prefetch. Therefore, use the terms in the ack that that are meant for the
      //prefetch.      
      mem_ack_gen_snack_valid = memtodr_ack_ff_valid; 
    end else begin
      mem_ack_gen_snack_valid = memtodr_ack_ff_valid && id_ram_valid; 
    end
  end
  ...
  assign mem_ack_gen_snack.snack = memtodr_ack_ff.ack;
  assign mem_ack_gen_snack.line =  memtodr_ack_ff.line;
