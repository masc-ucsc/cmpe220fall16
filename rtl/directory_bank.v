...
`ifndef DR_PASSTHROUGH
  ...
  assign mem_ack_gen_req_retry = tag_req_ff_stage_next_retry || tag_bank_read_next_retry || l2todr_req_gen_valid;
  assign snoop_ack_gen_in_req_retry = tag_req_ff_stage_next_retry || tag_bank_read_next_retry || l2todr_req_gen_valid || mem_ack_gen_req_valid;
  ...
  //or a disp. Included: snoop_ack_gen_in_req_retry
  always_comb begin
    ...
    end else if(mem_ack_gen_req_valid) begin
      tag_req_ff_stage_next = mem_ack_gen_req;
      tag_req_ff_stage_next_valid = !tag_bank_read_next_retry;
  ...
  always_comb begin
    ...
    end else if(l2todr_req_gen_valid || mem_ack_gen_req_valid || snoop_ack_gen_in_req_valid) begin
...
//SNOOP ACK START
  ...
  assign snoop_ack_gen_in_req.cmd = sa_buf.cmd;
  ...
  logic [`NUM_ENTRY_POINTERS-1:0] valid_entry_pointers;
  always_comb begin
...
//The ack is more complicated because we have to wait for a read on the RAM. 
  ...
  //An ack can also generate a request. This occurs when an outstanding request is conflicting causing it to be buffered.
  //We can detect if another request has been buffered here and start that request.
  I_dr_pipe_stage_type    mem_ack_gen_req;
  logic                   mem_ack_gen_req_valid;
  logic                   mem_ack_gen_req_retry;
  
  
  I_dr_req_buf_type       next_request;
  assign next_request = req_buf[req_buf[ack_buf_pos].next[`OUTSTANDING_REQUEST_BITS-1:0] - 2'b1];
  
  always_comb begin
    if(memtodr_ack.drid != {`DR_REQIDBITS{1'b0}}) begin 
      //Case only occurs when the ack is not caused by a prefetch.
      mem_ack_gen_req.paddr = next_request.paddr;
      mem_ack_gen_req.cmd   = next_request.cmd;
      mem_ack_gen_req.drid  = {3'b0,req_buf[ack_buf_pos].next};
      mem_ack_gen_req.nid = next_request.nid;
      mem_ack_gen_req.dr_cmd = 1'b0;
    end
  end
  
  always_comb begin
    mem_ack_gen_req_valid = 'b0;
    if(req_buf[ack_buf_pos].next != 'b0) begin
      mem_ack_gen_req_valid = memtodr_ack_valid && !(l2todr_req_valid && !l2todr_req_retry) && !mem_ack_gen_snack_retry;
    end
  end
