...
void advance_half_clock(Vram_1port_dense_wp *top) {
#ifdef TRACE
...
void advance_clock(Vram_1port_dense_wp *top, int nclocks=1) {
...
void error_found(Vram_1port_dense_wp *top) {
  advance_half_clock(top);
  advance_half_clock(top);
  sim_finish(false);
}

void try_send_packet(Vram_1port_dense_wp *top) {
  top->ack_retry = (rand()&0x3F)==0; 
...
void try_recv_packet(Vram_1port_dense_wp *top) {
...
int main(int argc, char **argv, char **env) {
  ...
  Vram_1port_dense_wp *top = new Vram_1port_dense_wp;
