...
void processor_t::disasm(insn_t insn)
{
  ...
  std::string insn_disasm;
  insn_disasm = disassembler->disassemble(insn).c_str();
  fprintf(stderr, "core %3d: 0x%016" PRIx64 " (0x%016" PRIx64 ") %s\n",
          id, state.pc, bits, insn_disasm.c_str());
}
