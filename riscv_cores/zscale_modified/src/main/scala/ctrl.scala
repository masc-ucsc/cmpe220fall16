...
import junctions.HastiConstants._
...
class CtrlDpathIO(implicit p: Parameters) extends ZscaleBundle {
  ...
  val id = new Bundle {
    ...
    val mem_type = UInt(OUTPUT, MemoryOpConstants.MT_SZ)
...
class Control(implicit p: Parameters) extends ZscaleModule()(p) {
  ...
  io.imem.hburst := HastiConstants.HBURST_SINGLE
  ...
  val imem_bus_error = io.imem.hready && io.imem.hresp === HastiConstants.HRESP_ERROR
  val dmem_bus_error = io.dmem.hready && io.dmem.hresp === HastiConstants.HRESP_ERROR
  ...
  io.dmem.hburst := HastiConstants.HBURST_SINGLE
